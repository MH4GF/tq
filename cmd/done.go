...
Structure it with these sections (omit any that don't apply):
  ...
  remaining:  Unfinished work — each entry that carries future work must
              reference a filed follow-up action: '- <what remains> → #<id>'
              (file it with 'tq action create … --task <id>'). Info-only
              notes belong in decisions/outcome, not here.
...
amend an already-"done" action's result, use 'tq action update ID --result'.`,
  ...
  remaining: API docs not yet updated for new token format → #210'`,
